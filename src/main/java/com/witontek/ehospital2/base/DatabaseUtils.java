...
public class DatabaseUtils {
	...
	public static void deleteDate(String databaseName, String sql) {
		...
		try {
			...
			c = DriverManager.getConnection(XmlUtils.getDataBaseXml().get(databaseName).getUrl(), XmlUtils.getDataBaseXml().get(databaseName)
					.getUsername(), XmlUtils.getDataBaseXml().get(databaseName).getPassword());
...
//			c = DriverManager.getConnection("jdbc:postgresql://121.40.154.101:5432/jnyxyfsyyadmin", "postgres", "P0stgreSql");
//			c.setAutoCommit(false);
//			stmt = c.createStatement();
